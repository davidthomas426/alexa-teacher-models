...
from alexa_teacher_models.configuration_atm import AlexaTMSeq2SeqConfig, AlexaTMConfig
...
class AlexaTMSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.scaling = 1.0 / math.sqrt(self.attention_head_size)
    ...
    def forward(
        ...
        if self.fp32_cast_query_key is not None:
        ...
        attention_probs = torch.softmax(attention_scores.to(hidden_states.dtype), dim=-1)
...
class AlexaTMLayer(nn.Module):
    def __init__(self, config):
        ...
        if self.add_cross_attention:
            ...
            logger.error(
                "skip/layer norm code is not tested for cross attention. "
...
class AlexaTMPreTrained(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, (nn.Linear, nn.Embedding)):
            ...
            if (isinstance(module, AlexaTMSelfOutput) and hasattr(module, 'dense')) or (
                isinstance(module, AlexaTMOutput) and hasattr(module, 'dense')
            ):
...
class AlexaTMSeq2SeqAttention(nn.Module):
    ...
    def __init__(
        ...
        bias: bool = True,
        fp32_cast_query_key: Optional[str] = None,
    ):
        super().__init__()
    ...
    def forward(
        ...
        attn_weights = torch.softmax(attn_weights.to(hidden_states.dtype), dim=-1)
...
class AlexaTMSeq2SeqDecoderLayer(nn.Module):
    def __init__(self, config: AlexaTMSeq2SeqConfig):
        ...
        self.self_attn_layer_norm = AlexaTMLayerNorm(
            config.d_model, eps=config.layer_norm_eps, fp32_cast_layer_norm=config.fp32_cast_layer_norm
        )
        ...
        self.encoder_attn_layer_norm = AlexaTMLayerNorm(
            config.d_model, eps=config.layer_norm_eps, fp32_cast_layer_norm=config.fp32_cast_layer_norm
        )
        self.fc1 = nn.Linear(self.embed_dim, config.decoder_ffn_dim)
        self.fc2 = nn.Linear(config.decoder_ffn_dim, self.embed_dim)
        self.final_layer_norm = AlexaTMLayerNorm(
            config.d_model, eps=config.layer_norm_eps, fp32_cast_layer_norm=config.fp32_cast_layer_norm
        )
...
class AlexaTMSeq2SeqDecoder(AlexaTMSeq2SeqPretrainedModel):
    ...
    def __init__(self, config: AlexaTMSeq2SeqConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.embed_positions = AlexaTMSeq2SeqLearnedPositionalEmbedding(config.max_position_embeddings, config.d_model)
        self.layers = nn.ModuleList([AlexaTMSeq2SeqDecoderLayer(config) for _ in range(config.decoder_layers)])
        self.layernorm_output = AlexaTMLayerNorm(
            config.d_model, eps=config.layer_norm_eps, fp32_cast_layer_norm=config.fp32_cast_layer_norm
        )
...
class AlexaTMSeq2SeqModel(AlexaTMSeq2SeqPretrainedModel):
    ...
    def parallelize(self, num_devices, start_device):
        ...
        assert num_devices + start_device <= torch.cuda.device_count(), (
            "number of available GPUs are less than " "num_devices plus start_device for " "model parallel"
        )
        device_list = ['cuda:{}'.format(i) for i in range(start_device, num_devices + start_device)]
        self.set_model_parallel_devices(num_devices)
        if num_devices == 2:
...
class AlexaTMSeq2SeqForConditionalGeneration(AlexaTMSeq2SeqPretrainedModel):
    ...
    def _prepare_decoder_input_ids_for_generation(
        ...
        if isinstance(input_ids, int):
            ...
            decoder_input_ids = (
                torch.ones((batch_size, 1), dtype=torch.long, device=self.model.decoder.starting_device)
                * decoder_start_token_id
            )
        else:
            decoder_input_ids = (
                torch.ones((input_ids.shape[0], 1), dtype=input_ids.dtype, device=self.model.decoder.starting_device)
                * decoder_start_token_id
            )
...
def register_auto_model():
    ...
    AutoConfig.register("alexatm", AlexaTMSeq2SeqConfig)
    AutoModelForSeq2SeqLM.register(AlexaTMSeq2SeqConfig, AlexaTMSeq2SeqForConditionalGeneration)
